...
class EntityLike(Listener):  # 实体类
    ...
    def draw(self, camera: tuple[int, int]):  # 定义显示实体的方法，该方法在场景需要描绘图像的时候调用
        rect = self.rect.move(*(-i for i in camera))  # 根据摄像头的位置计算实际要描绘的位置，例如摄像头往上了，实际描绘的位置就要往下
...
class Player(EntityLike):  # 玩家类
    ...
    def listen(self, event: Event):  # 玩家类所响应的事件
        if event.code == pygame.KEYDOWN:  # 键盘按下事件
            ...
            self.rect.y = event.body["POS"][1]# 继承原有的响应事件内容，如对DRAW的响应
    ...
    def __init__(self, rect: pygame.rect.Rect):
        ...
        super().__init__(
            image=pygame.transform.scale(
                pygame.image.load("C:/Users/pc/Desktop/si100/project/PVZ个人创作/images/helmet.png"), (40, 40)
            ),
            rect=rect,
...
class Tile(EntityLike):  # 地图背景的类，背景是由一个一个方块组成的，每个方块都是该类的实例
    def __init__(self, type: int, rect: pygame.rect.Rect):
        ...
        super().__init__(
            image=pygame.transform.scale(pygame.image.load("C:/Users/pc/Desktop/si100/project/PVZ个人创作/images/gameover.png"),(40, 40),),
            rect=rect,
        )
...
class SceneLike(Listener):  # 场景的类，管理障碍物、角色、地图背景的描绘、刷新等
    ...
    def listen(self, event: Event):  # 场景所监听的事件
        if event.code == REQUEST_MOVE:  # 监听玩家的移动请求事件
            ...
            target_rect = pygame.Rect(event.body["POS"][0], event.body["POS"][1], 60, 60
)  # 获得移动后的玩家矩形
...
if __name__ == "__main__":
    mob = Player(image=pygame.transform.scale(pygame.image.load("C:/Users/pc/Desktop/si100/project/PVZ个人创作/images/zombie2.gif"), (60, 60)),
                 rect=pygame.Rect(610, 330, 60, 60),)  # 生成角色实例
