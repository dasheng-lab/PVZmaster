...
class Card:
    def __init__(self):
        ...
        self.sunshine = self.initsunshine
        self.active = 0
        self.suncount = 0
        ...
        self.sundiex = 0
        self.place = [0, 0]
        self.sunrect = pygame.Rect(random.randint(100, 800), 0, 50, 50)
    ...
    def draw(self):
        ...
        for i in [1, 3, 4, 5, 6, 7]:
            ...
            if self.plant[i]:
                if (
                    self.plant_cold[i] >= self.recold[i]
        ...
        if self.suncount >= 500:
            self.suncount = 0
            self.gen_sun = True
            self.sunrect = pygame.Rect(random.randint(100, 800), 50, 50, 50)
        if self.gen_sun:
            self.sundiex += 0.2
            self.sunrect.y += 2
            sun_blit = int(self.sundiex % len(sun_frame))
            if self.sundiex < 7:
                sun = Fallthing(
                    sun_frame[sun_blit], self.sunrect.x, self.sunrect.y, 40, 40
                )
                sun.draw([0, 0])
            elif self.sundiex < 14:
                self.place[0] += d(self.sunrect.x, self.sunrect.y, 225, -10)[0]
                self.place[1] += d(self.sunrect.x, self.sunrect.y, 225, -10)[1]
                sun = Fallthing(
                    sun_frame[sun_blit],
                    self.place[0] + self.sunrect.x,
                    self.place[1] + self.sunrect.y,
                    40,
                    40,
                )
                sun.draw([0, 0])
                ...
                self.place = [0, 0]
                self.sunshine += 25
                audio_player.play_sound("收集阳光", 0, 0.8)
        ...
        if self.active == 0:
            for i in [1, 3, 4, 5, 6, 7]:
                if (
                    ...
                    audio_player.play_sound("选中", 0, 0.6)
                    self.active = i
                    self.clicked = 0
                else:
                    self.clicked += 1
                ...
                if lawn_dict[lawn].rect.collidepoint(mouse_pos):
                    if self.active == 1:  # 向日葵
                        if lawn_avaible[lawn]:
                            ...
                            audio_player.play_sound("种植物", 0, 0.6)
                        else:
                            self.active = 0
                    if self.active == 3:  # 坚果
                        if lawn_avaible[lawn]:
...
class Sunflower(Plant):
    ...
    def draw(self, camera, style=0):
        ...
        if self.kind == 1:
            ...
            self.rect.y = self.y - camera[1] + 10
            screen.blit(self.image, self.rect)
            sun_blit = int(self.pic_diex % len(sun_frame))
            if self.sundiex < 7:
                sun = Fallthing(
                    sun_frame[sun_blit],
                    self.rect.x - f(self.sundiex * 2),
                    self.rect.y + g(self.sundiex * 2, 0.5),
                    40,
                    40,
                )
                sun.draw(camera)
            elif self.sundiex < 14:
                self.place[0] += d(self.rect.x, self.rect.y, 240, -10)[0]
                self.place[1] += d(self.rect.x, self.rect.y, 240, -10)[1]
                sun = Fallthing(
                    sun_frame[sun_blit],
                    self.place[0] + self.rect.x - f(14) + camera[0],
                    self.place[1] + self.rect.y + g(14, 0.5) + camera[1],
                    40,
                    40,
                )
                sun.draw(camera)
        ...
        if self.funx == 0:
            for zombie in emg.zombie_list:
                if (
                    ...
                    audio_player.play_sound("窝瓜怀疑", 0, 0.5)
                    self.funx = 1
        ...
        if self.funx == 0:
            for zombie in emg.zombie_list:
                if (
                    ...
                    zombie.collide = True
                if zombie.tag == 1 and zombie.tag == 5:
                    if (
                        pygame.Rect(
                            self.rect.x + 20,
                            self.rect.y - (self.rect.width - 65) + 20,
...
class Level(Listener):
    ...
    def level_start(self):
        self.shout = random.randint(0, 10000)
        if self.shout <= 5 and emg.zombie_list != []:
            audio_player.play_sound("僵尸叫", 0)
        if self.startdealing <= 670:
            self.startdealing += 1
            if self.startdealing == 670:
                audio_player.play_sound("第一波", 0)
                pass  # 这里放第一个僵尸来的音效
        if self.startdealing > 670:
            if self.level == 1:
                if (
                    ...
                    self.is_huge = False
                    self.count = 0
                    for i in range(1, self.hardness + 1):
                    ...
                    self.count > 2000 / self.hardness or emg.zombie_list == []
                ) and int(self.win_count) == 15:
                    self.waitdealing += 1
                    if self.waitdealing == 1:
                        self.hugewavedealing = 1
                    if self.waitdealing == 200:
                        emg.AIput(player.rect.x, player.rect.y, 1, 2)
                    elif self.waitdealing == 250:
                        self.waitdealing = 0
                        self.count = 0
                        for i in range(1, self.hardness + 1):
                ...
                if (
                    ...
                    self.count > 1000 or emg.zombie_list == []
                ) and self.win_count < 20:
                    sum = 0
                    while sum < 7 * self.hardness:
                        add = random.randint(1, 2)
                        emg.AIput(player.rect.x, player.rect.y, add)
                        sum += add
                    self.count = 0
                ...
                if (
                    ...
                    self.count = 0
                    sum = 0
                    while sum < 24 * self.hardness:
                        add = random.randint(1, 4)
                        if add != 3:
                            emg.AIput(player.rect.x, player.rect.y, add)
                        else:
                            subadd = random.randint(1, 2)
                            emg.AIput(player.rect.x, player.rect.y, add, subadd)
                        sum += add
                    self.win_count += 1
                elif (
                    self.count > 1000 / self.hardness or emg.zombie_list == []
                ) and self.win_count < 35:
                    self.count = 0
                    sum = 0
                    while sum < 28 * self.hardness:
                        add = random.randint(1, 4)
                        if add != 3:
                            emg.AIput(player.rect.x, player.rect.y, add)
                        else:
                ...
                if (
                    ...
                    self.count > 1000 / self.hardness or emg.zombie_list == []
                ) and int(self.win_count) < 36:
                    self.count = 0
                    sum = 0
                    while sum < 32 * self.hardness:
                        add = random.randint(1, 4)
                        if add != 3:
                            emg.AIput(player.rect.x, player.rect.y, add)
                        else:
                    ...
                    self.count > 1000 / self.hardness or emg.zombie_list == []
                ) and int(self.win_count) < 40:
                    self.count = 0
                    sum = 0
                    while sum < 36 * self.hardness:
                        add = random.randint(1, 4)
                        if add != 3:
                            emg.AIput(player.rect.x, player.rect.y, add)
                        else:
        ...
        if self.finaldealing >= 1 and self.finaldealing <= 130:
            ...
            if self.finaldealing == 1:
                audio_player.play_sound("最后一波", 0)
                pass  # 这里放最后一波僵尸来的音效
            self.finaldealing += 1
        ...
        if moneybox.is_clicked() and self.is_draw:
            ...
            player_money.add_money(
                200 + 50 * self.level + int(0.025 * card_box.sunshine)
            )
            self.post(
                Event(
        ...
        if wincup.is_clicked() and self.wincupis_draw:
            ...
            audio_player.play_sound("通关音乐", 0)
            self.win_count = 0
            self.level = 1
            self.triumph = False
            self.wincupis_draw = False
