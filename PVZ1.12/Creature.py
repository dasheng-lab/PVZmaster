...
class ZombieManager(pygame.sprite.Sprite):
    ...
    def gen_new_flag_zombie(self, gen):
        self.zombie_list.append(Flag_Zombie(950, gen))

    def gen_new_paper_zombie(self, gen):
        self.zombie_list.append(Paper_Zombie(950, gen))
...
class Zombie(pygame.sprite.Sprite):
    ...
    def draw(self, camera):
        ...
        if self.style == 0:  # 正常移动
            ...
            story=copy.copy(self.image)
            screen.blit(changecolor(story,0.5,0.5,0.5), (self.rect.x - camera[0], self.rect.y - camera[1]))
...
class Rugby_Zombie(pygame.sprite.Sprite):
    ...
    def move(self, default=0):
        if self.style != 9 and not (self.style == 3 and self.eating) and self.style != 10:
            self.x -= self.move_speed / (self.snow + 1) + default
