...
class Roadblock_Zombie(pygame.sprite.Sprite):
    ...
    def draw(self, camera):
        if self.snowcount > 0:
            ...
            self.snow = 0
        if self.style == 0:  # 行走
            self.walkindex += 0.2 / (self.snow + 1)
            ...
            if not self.hit:
                ...
                eatblit = int(self.eatindex % len(roadblockeat_frame0[self.snow]))
                screen.blit(
                    roadblockeat_frame[self.snow][eatblit],
                    (self.rect.x - camera[0], self.rect.y - camera[1]),
                )
                if not pygame.Rect(
                    player.rect.x - camera[0],
                ...
                self.eatindex += 0.2 / (self.snow + 1)
                eatblit = int(self.eatindex % len(roadblockeat_frame[self.snow]))
                self.rect.x = self.x
                self.rect.y = screen_height - (
                    self.y + roadblockeat_frame0[self.snow][eatblit].get_height()
                )
...
class Rugby_Zombie(pygame.sprite.Sprite):
    ...
    def draw(self, camera):
        ...
        if self.style == 0:  # 正常移动
            ...
            if self.dealing == 0:
                if self.HP > 500:
                    self.image = changecolor(self.image, 0.7, 0.7, 0.7)
                else:
                    self.image2 = changecolor(self.image2, 0.7, 0.7, 0.7)
                self.dealing = 1
            if self.HP > 500:
                screen.blit(
                    self.image, (self.rect.x - camera[0], self.rect.y - camera[1])
            ...
            screen.blit(
                rugbynocap_frame0[self.snow][walkblit],
                (
                    self.rect.x - camera[0] + 15,
                    self.rect.y - camera[1] + 20,
                    self.rect.width,
                    self.rect.height,
                ),
            )
