...
NPC_picture={"1":"images/player's wife/图层 1.png",
...
class Talk_content():#NPC对话
    ...
    def dialog_text(self, str):
        ...
        for line in self._dialog_text:
            ...
            text_rect = text_surface.get_rect(center=(text_surface.get_width() // 2+20, self.y_offset+500))
...
class Text(EntityLike):#文本框输入
    ...
    def draw(self):
        ...
        if self.active and self.cursor:#画光标
            ...
            pygame.draw.line(self.surf,(0,0,0),(x,self.y+5),(x,self.y+self.rect.height-5),1)
        if self.delete:
            try:
                self.list.pop()
                self.delete=False
